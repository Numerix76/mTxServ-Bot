...
module.exports = {
	run: async (member) => {
		...
		const embed = new Discord.MessageEmbed()
			.setColor('BLUE')
			.setDescription(`**${member.user.tag}** joined using invite code \`${invitation.code}\` from **${invitation.creatorName}**. Invite was used **${invitation.uses} times** since its creation.`)
			.setTimestamp();
